...
class Card:
    ...
    def ask_players(self, current_player):
        if len(self.game.players) > 2:
            current_player.bot.send_message(current_player.chat.id, Strings.ASK_DESIGNATE,
    ...
    def ask_ten(self, player):
        if self.game.value < 10:
            ...
            player.bot.send_message(player.chat.id, Strings.ASK_TEN,
                    reply_markup = InlineKeyboardMarkup([[
    ...
    def ask_twenty(self, player):
        if self.game.value < 20:
            ...
            player.bot.send_message(player.chat.id, Strings.ASK_TWENTY,
                    reply_markup = InlineKeyboardMarkup([[
    ...
    def choose(self, player, idx):
        if self.rank == 5:
            ...
            self.game.bot.send_message(Settings.CHAT_ID, Strings.DESIGNATED.format('@' + player.user.username, '@' + self.game.players[idx].user.username))
        elif self.rank == 10:
            if idx == 0:
                self.game.value += 10
            elif idx == 1:
